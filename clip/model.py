...
class ResidualAttentionBlock(nn.Module):
    ...
    def forward(self, inputs):
        # x: (N, B, C)
        x, space_dict, temperature, sd_ft_all, max_keep, feat_all = inputs
        ...
        if feat_all is not None:
            feat_all += (x,)
        return x, space_dict, temperature, sd_ft_all, max_keep, feat_all
...
class Transformer(nn.Module):
    ...
    def forward(self, x: torch.Tensor, space_dict=None, temperature=0, sd_ft_all=None, max_keep=1, feat_all=None):
        return self.resblocks((x, space_dict, temperature, sd_ft_all, max_keep, feat_all))
...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x: torch.Tensor, space_dict=None, temperature=0, max_keep=1):
        ...
        if space_dict is not None:
            x, _, _, sd_img_ft_all, _, vis_feat_all = self.transformer(x, space_dict, temperature, sd_img_ft_all, max_keep, vis_feat_all)
        else:
        ...
        return x, sd_img_ft_all, vis_feat_all
...
class CLIP(nn.Module):
    ...
    def encode_text(self, text, space_dict=None, temperature=0):
        ...
        x, _, _, sd_txt_ft_all, _, lang_feat_all = self.transformer(x, space_dict, temperature, sd_txt_ft_all, max_keep, lang_feat_all)
        ...
        return x, sd_txt_ft_all, lang_feat_all
    ...
    def forward(self, image, caption, alpha, idx, temperature=0):
        ...
        image_features, sd_img_ft, vis_feat_all = self.encode_image(image, space_dict=self.space_dict, temperature=temperature)
        text = self.tokenize(caption).to(image.device)
        text_features, sd_txt_ft, lang_feat_all = self.encode_text(text, space_dict=self.space_dict, temperature=temperature)
        ...
        with torch.no_grad():
            ...
            image_features_m, sd_img_ft_m = self.encode_image_m(image, space_dict=self.space_dict, temperature=temperature)[:2]
            text_features_m, sd_txt_ft_m = self.encode_text_m(text, space_dict=self.space_dict, temperature=temperature)[:2]
        ...
        return loss_ita, loss_fdt, loss_fdt_m, vis_feat_all, lang_feat_all
