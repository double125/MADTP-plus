...
class BLIP_NLVR(nn.Module):
    ...
    def forward(self, image, text, targets, temperature=0.5, train=False):
        ...
        output, sd_txt_ft, lang_feat_all = self.text_encoder(text.input_ids, 
        ...
        if train:            
            ...
            return loss_ori, loss_fdt, prediction, vis_feat_all, lang_feat_all,
        else:
            return prediction, vis_feat_all, lang_feat_all,
            # return prediction

    # def forward(self, image, input_ids, input_mask, temperature=0, targets=None, train=False):
    #     image_embeds, sd_img_ft, vis_feat_all = self.visual_encoder(image, space_dict=self.space_dict, temperature=temperature)
    #     image_atts = torch.ones(image_embeds.size()[:-1],dtype=torch.long).to(image.device)        
    #     image0_embeds, image1_embeds = torch.split(image_embeds, image_embeds.size(0) / 2)     
    #     # text = self.tokenizer(text, padding='longest', return_tensors="pt").to(image.device) 
    #     # text.input_ids[:,0] = self.tokenizer.enc_token_id  

    #     output, sd_txt_ft, lang_feat_all = self.text_encoder(input_ids, 
