...
class BLIP_Retrieval(nn.Module):
    ...
    def forward(self, image, caption, alpha, idx, temperature=0, train=True):
        ...
        image_embeds, sd_img_ft, vis_feat_all = self.visual_encoder(image, space_dict=self.space_dict, temperature=temperature) 
        ...
        text_output, sd_txt_ft, lang_feat_all = self.text_encoder(text.input_ids, attention_mask = text.attention_mask,                      
        ...
        with torch.no_grad():
            ...
            image_embeds_m, sd_img_ft_m, _ = self.visual_encoder_m(image, space_dict=self.space_dict, temperature=temperature) 
            image_feat_m = F.normalize(self.vision_proj_m(image_embeds_m[:,0,:]),dim=-1)  
            image_feat_m_all = torch.cat([image_feat_m.t(),self.image_queue.clone().detach()],dim=1)                   
            
            text_output_m, sd_txt_ft_m, _ = self.text_encoder_m(text.input_ids, attention_mask = text.attention_mask,                      
        ...
        bs = image.size(0)
        output_pos, _, _ = self.text_encoder(encoder_input_ids,
        ...
        output_neg, _, _ = self.text_encoder(text_ids_all,
        ...
        return loss_ita, loss_itm, loss_fdt, loss_fdt_m, vl_output, vis_feat_all, lang_feat_all 
